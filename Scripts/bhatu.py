...
def wrappingFunction(I, W, Tpoints):
    ...
    transformedImageIntensities[:, 0] = I[
        transformedImagePoints[:, 1].astype(int), transformedImagePoints[:, 0].astype(int)]
...
# Taking user input
inp = input(
    "Do you want to select the bounding box for template or use the tested bounding box for best result?('y' for Yes and 'n' for no) :")
if (inp == "y"):
...
for img in imgs:
    ...
    # LucasKanadeTracker
    # updatedParam,p = pyramidLucasKanade(frame1Points, frame1Intensities, frame2, p, startingPoint, endPoint)
